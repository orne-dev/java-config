...
/**
 * Unit tests for {@link JsonMutableConfig}.
 * 
...
extends AbstractWatchableConfigTest {
    ...
    throws IOException {
        ...
        final ObjectNode fileProps = NODE_FACTORY.objectNode()
                .set("test", NODE_FACTORY.objectNode()
                        ...
                        .set("file", NODE_FACTORY.objectNode()
                            .put("type", TEST_FILE_TYPE)));
        testFile = File.createTempFile(JsonMutableConfigTest.class.getSimpleName(), ".json");
        ...
        testUrl = JsonMutableConfigTest.class.getResource("test.url.json");
    }
    ...
    void testEmptyBuilder() {
        final JsonMutableConfig config = Config.fromJsonFiles()
                .mutable()
                .build();
    ...
    /**
     * Test method for {@link JsonMutableConfig#JsonMutableConfig(ConfigOptions, MutableConfigOptions, JsonConfigOptions)}.
     */
    @Test
    void testOptionsConstructor() {
        final ConfigOptions baseOptions = mock(ConfigOptions.class);
        final MutableConfigOptions mutableOptions = mock(MutableConfigOptions.class);
        ...
        given(mutableOptions.getEncoder()).willReturn(mockEncoder);
        given(jsonOptions.getJsonObject()).willReturn(mockJsonObject);
        given(jsonOptions.getPropertySeparator()).willReturn(mockSeparator);
        
        assertThrows(NullPointerException.class, () -> {
            new JsonMutableConfig(null, null, null);
        });
        assertThrows(NullPointerException.class, () -> {
            new JsonMutableConfig(null, mutableOptions, jsonOptions);
        });
        assertThrows(NullPointerException.class, () -> {
            new JsonMutableConfig(baseOptions, null, jsonOptions);
        });
        assertThrows(NullPointerException.class, () -> {
            new JsonMutableConfig(baseOptions, mutableOptions, null);
        });
        
        final JsonMutableConfig config = new JsonMutableConfig(
                baseOptions,
                mutableOptions,
                jsonOptions);
        assertSame(mockParent, config.getParent());
        assertSame(mockDecoder, config.getDecoder());
        assertSame(mockEncoder, config.getEncoder());
        assertSame(mockDecorator, config.getDecorator());
        assertSame(mockJsonObject, config.getJsonObject());
    ...
    void testComponentsConstructor() {
        final String mockSeparator = "mock.separator";
        assertThrows(NullPointerException.class, () -> {
            new JsonMutableConfig(null, null, null, null, null, null);
        });
        assertThrows(NullPointerException.class, () -> {
            new JsonMutableConfig(mockParent, null, mockEncoder, mockDecorator, mockJsonObject, mockSeparator);
        });
        assertThrows(NullPointerException.class, () -> {
            new JsonMutableConfig(mockParent, mockDecoder, null, mockDecorator, mockJsonObject, mockSeparator);
        });
        assertThrows(NullPointerException.class, () -> {
            new JsonMutableConfig(mockParent, mockDecoder, mockEncoder, null, mockJsonObject, mockSeparator);
        });
        assertThrows(NullPointerException.class, () -> {
            new JsonMutableConfig(mockParent, mockDecoder, mockEncoder, mockDecorator, null, mockSeparator);
        });
        assertThrows(NullPointerException.class, () -> {
            new JsonMutableConfig(null, mockDecoder, mockEncoder, mockDecorator, mockJsonObject, null);
        });
        assertDoesNotThrow(() -> {
            new JsonMutableConfig(null, mockDecoder, mockEncoder, mockDecorator, mockJsonObject, mockSeparator);
        });
